...
- **Component rendering.** Vitest tests cover pure logic only — no jsdom, no React Testing Library. A JSX bug in `NdaWorkspace.tsx`, `UnsupportedDocWorkspace.tsx`, `NdaPreview.tsx`, or `DownloadPdfButton.tsx` would slip past `npm test`.
